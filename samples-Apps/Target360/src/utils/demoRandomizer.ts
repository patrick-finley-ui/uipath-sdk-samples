import type { TargetInvestigationEntity, RiskLevel } from '../types/investigation';
 ...
 */
function getSeededIndex(seed: string, arrayLength: number, offset: number = 0): number {
  const combinedSeed = seed + offset.toString();
  return Math.floor(seededRandom(combinedSeed) * arrayLength);
}
 ...
 */
function getRandomName(recordId: string): { first: string; last: string; nationality: string } {
  const index = getSeededIndex(recordId, DEMO_NAMES.length);
 ...
 */
function getRandomNationality(recordId: string): string {
  const index = getSeededIndex(recordId, NATIONALITIES.length, 1);
 ...
 */
function getRandomRiskLevel(recordId: string): RiskLevel {
  const random = seededRandom(recordId + 'risk');
  let cumulative = 0;
 ...
 */
function getFlaggedChecks(recordId: string, risk: RiskLevel): number {
  const random = seededRandom(recordId + 'checks');
  if (risk === 'High') {
...
export function shouldRandomize(
  ...
  const entityTime = entity.UpdateTime || entity.CreateTime;
  if (!entityTime) return false; // If no timestamp, don't randomize (safe default)

  return new Date(entityTime) < new Date(demoResetTime);
}

/**
 * Apply randomization to entity fields (creates new object, non-mutating)
 * Randomizes: subjectName, subjectNationality, risk, numChecksFlagged
...
export function randomizeEntity(
  entity: TargetInvestigationEntity,
  demoResetTime: string | null
): TargetInvestigationEntity & { isRandomized?: boolean } {
  if (!shouldRandomize(entity, demoResetTime)) {
    return { ...entity, isRandomized: false };
  }

  const recordId = entity.Id || 'unknown';
  const name = getRandomName(recordId);
  const risk = getRandomRiskLevel(recordId);
  const nationality = getRandomNationality(recordId);
  const flaggedChecks = getFlaggedChecks(recordId, risk);
