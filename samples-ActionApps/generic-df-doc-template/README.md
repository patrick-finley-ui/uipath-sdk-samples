# Generic Task Template - Action App

A modern, fully configurable task template built with React, Vite, and Tailwind CSS. This Action App integrates with UiPath Action Center and Data Fabric to provide a generic workflow for reviewing and processing any entity type (loan applications, invoices, claims, contracts, etc.).

## Features

- 🔧 **Fully Configurable**: Works with any entity type through input parameters
- 📊 **Dynamic Entity History**: View entity records from Data Fabric with configurable fields
- 📄 **Document Management**: View and filter documents from Orchestrator Buckets
- ✅ **Flexible Outcomes**: Configurable action buttons for different workflow outcomes
- 🎨 **Modern UI**: Built with Tailwind CSS and shadcn/ui components
- 🤖 **AI Integration**: Display AI agent analysis and recommendations
- 📝 **Dynamic Forms**: Configurable read-only and input fields
...
3. **Test with mock data:**
   ...
   - See `examples/QUICK_START.md` for testing instructions

### Example Configurations

Five example configurations are provided in the `examples/` folder:
- `loan-application-config.json` - Loan application review
- `invoice-config.json` - Invoice approval workflow
- `insurance-claim-config.json` - Insurance claim processing
- `contract-review-config.json` - Contract review and approval
- `employee-onboarding-config.json` - Employee onboarding workflow
