...
const Form = () => {
  ...
  // Store invoice history data fetched from UiPath Data Fabric
    ...
    sdk.taskEvents.getTaskDetailsFromActionCenter((data: any) => {
      ...
      if (data.data) {
        const processedData = {
          ...
          // Ensure decision is always a string (never null/undefined) to keep Select controlled
  ...
  // ============================================================================
  // LIFECYCLE HOOK: Load Invoice History from Data Fabric (API Call)
  // ============================================================================
  // This useEffect runs whenever the user switches tabs. It loads invoice history
  // data from UiPath Data Fabric when the "Invoice History" tab is clicked.
    ...
    if (activeTab === 'applicant' && !hasLoadedHistory && !isLoadingHistory) {
      const loadInvoiceHistory = async () => {
        try {
          ...
          // API CALL: Fetch invoice records from UiPath Data Fabric
          // The entity ID '8095dc2e-c2f3-f011-832f-000d3abf6e1a' is the Data Fabric entity
          const response = await sdk.entities.getRecordsById('8095dc2e-c2f3-f011-832f-000d3abf6e1a', {
            pageSize: 5,           // Get up to 5 records
            expansionLevel: 1      // Include related data
          });
          console.log('Invoice history response:', response);
          ...
          if (response && response.items) {
            const mappedHistory = response.items.map((record: any, index: number) => ({
              id: index + 1,
              invoiceNumber: record.InvoiceNumber || record.invoiceNumber || 'N/A',
              vendor: record.Vendor || record.vendor || 'N/A',
              lineItemCount: record.LineItemCount || record.lineItemCount || 0,
              totalAmount: record.TotalAmount || record.totalAmount || 0,
              invoiceDate: record.InvoiceDate || record.invoiceDate || new Date().toISOString(),
              dueDate: record.DueDate || record.dueDate || new Date().toISOString(),
              status: record.Status || record.status || 'Unknown'
            }));
            setInvoiceHistory(mappedHistory); // Update state with fetched data
          }
          setHasLoadedHistory(true); // Mark as loaded so we don't load again
        } catch (error) {
          console.error('Error loading invoice history:', error);
          setInvoiceHistory([]); // Set empty array on error
      ...
      loadInvoiceHistory(); // Execute the async function
    }
  ...
  // Handle "Approve Invoice" button click
  const handleApprove = () => {
    console.log('Invoice approved:', formData);
    // Complete the task in Action Center with "Approve Invoice" outcome and send form data
    sdk.taskEvents.completeTask('Approve Invoice', formData);
  };

  // Handle "Reject Invoice" button click
  const handleReject = () => {
    console.log('Invoice rejected:', formData);
    // Complete the task in Action Center with "Reject Invoice" outcome and send form data
    sdk.taskEvents.completeTask('Reject Invoice', formData);
  };

  // Handle "Request Correction" button click
  const handleRequestCorrection = () => {
    console.log('Correction requested:', formData);
    // Complete the task in Action Center with "Request Correction" outcome and send form data
    sdk.taskEvents.completeTask('Request Correction', formData);
  };
          ...
          <div className="space-y-8">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div>
                    ...
                    <div className="bg-muted px-4 py-3 rounded-lg font-medium">
                      {formData.invoiceAmount ? `$${parseFloat(formData.invoiceAmount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'N/A'}
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-muted-foreground mb-2">
            ...
            <Card className="border-2 border-primary/20">
              ...
              <CardContent className="space-y-6">
                <div>
                  <label htmlFor="decision" className="block text-sm font-semibold mb-2">Decision *</label>
                  <Select value={formData.decision} onValueChange={handleSelectChange}>
                    <SelectTrigger className="w-full h-11">
                      <SelectValue placeholder="Select Decision" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="Approve Invoice">Approve Invoice</SelectItem>
                      <SelectItem value="Reject Invoice">Reject Invoice</SelectItem>
                      <SelectItem value="Request Correction">Request Correction</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div>
            ...
            <div className="flex gap-4">
              <Button type="button" onClick={handleApprove} disabled={!isFormValid} size="lg" className="flex-1 h-14 text-lg bg-primary hover:opacity-90 text-primary-foreground">Approve Invoice</Button>
              <Button type="button" onClick={handleReject} disabled={!isFormValid} variant="destructive" size="lg" className="flex-1 h-14 text-lg">Reject Invoice</Button>
              <Button type="button" onClick={handleRequestCorrection} disabled={!isFormValid} variant="outline" size="lg" className="flex-1 h-14 text-lg">Request Correction</Button>
            </div>
          </div>
        );

      case 'applicant':
        const totalHistoryPages = Math.ceil(invoiceHistory.length / historyPerPage);
        const startHistoryIndex = (historyPage - 1) * historyPerPage;
        const endHistoryIndex = startHistoryIndex + historyPerPage;
        const paginatedHistory = invoiceHistory.slice(startHistoryIndex, endHistoryIndex);
          ...
          <Card className="border-2">
            ...
            <CardContent className="space-y-4">
              {isLoadingHistory ? (
                ...
                <div className="text-center py-12 text-muted-foreground bg-muted rounded-lg">No invoice history available in Data Fabric</div>
              ) : (
                <>
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>Line Item Count</TableHead>
                    ...
                    <TableBody>
                      {paginatedHistory.map((invoice) => (
                        <TableRow key={invoice.id}>
                          ...
                          <TableCell>${invoice.totalAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
                          <TableCell>{invoice.invoiceDate}</TableCell>
                          <TableCell>{invoice.dueDate}</TableCell>
                          <TableCell>
                            <span className={cn("inline-block px-3 py-1 rounded-full text-xs font-medium", invoice.status.toLowerCase() === 'approved' && "bg-primary/20 text-primary", invoice.status.toLowerCase() === 'pending' && "bg-yellow-500/20 text-yellow-400", invoice.status.toLowerCase() === 'rejected' && "bg-destructive/20 text-destructive", !['approved', 'pending', 'rejected'].includes(invoice.status.toLowerCase()) && "bg-muted text-muted-foreground")}>
                              {invoice.status}
                            </span>
                          </TableCell>
                        </TableRow>
                  ...
                  {totalHistoryPages > 1 && (
                    <div className="flex items-center justify-between px-2">
                      <div className="text-sm text-muted-foreground">
                        Showing {startHistoryIndex + 1}-{Math.min(endHistoryIndex, invoiceHistory.length)} of {invoiceHistory.length} records
                      </div>
  ...
  // Decision is required, so buttons are disabled until it's filled
  const isFormValid = formData.decision && formData.decision !== '';
